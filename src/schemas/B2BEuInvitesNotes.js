...
// --- Begin: B2B Partner End Users Invitations Notes Schema --- //
const schema = new Schema({
  ...
  invUser: {type: String, required: true},
  invUserPrimary: {type: String, required: true},
  lead: {type: String, required: false}, // Leads._id
  leadId: {type: String, required: false}, // Leads.leadId
  ...
  type: {type: String, required: true}, // Invitations, On Boarding, In Marketing
...
schema.index({delFlag: -1, b2b: 1, type: 1, invite: 1, lead: 1});
schema.index({delFlag: -1, b2b: 1, invite: 1, cUser: 1});
schema.index({cDtStr: -1, uDtStr: -1});

module.exports = mongoose.model(config.collB2BEuInvitesNotes, schema);
// --- End: B2B Partner End Users Invitations Notes Schema --- //
